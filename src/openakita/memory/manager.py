...
class MemoryManager:
    ...
    def _ensure_vector_store(self) -> VectorStore | None:
        ...
        # Check if chromadb backend is enabled
        from ..config import settings

        if settings.search_backend != "chromadb":
            return None
        ...
        try:
            ...
            self._vector_store = VectorStore(
                ...
                download_source=settings.model_download_source,
            )
            self.vector_store = self._vector_store
            logger.info("[MemoryManager] VectorStore loaded on demand")
            return self._vector_store
        except Exception as e:
            logger.warning(f"[MemoryManager] Failed to load VectorStore: {e}")
            return None

        # VectorStore: only create when chromadb backend is selected
        if search_backend == "chromadb":
            # 懒加载 VectorStore，避免启动时加载 embedding 模型
            ...
            # v1 compat: in-memory cache (LRU 淘汰)
            self.memories_file = self.data_dir / "memories.json"
            self._cache_max_size = 10000  # 10000 条记忆上限
            self._cache_ttl_seconds = None  # None = 永不过期
            self._memories: OrderedDict[str, Memory] = OrderedDict()  # 使用 OrderedDict 实现 LRU
    ...
    def _add_to_cache(self, memory_id: str, memory: Memory) -> None:
        """添加到缓存（LRU 更新）"""
        with self._memories_lock:
            if memory_id in self._memories:
                self._memories.move_to_end(memory_id)  # 移到末尾（最近使用）

            self._memories[memory_id] = memory

            # 仅当超出上限时淘汰最旧的
            while len(self._memories) > self._cache_max_size:
                self._memories.popitem(last=False)  # 删除最旧

    def _get_from_cache(self, memory_id: str) -> Memory | None:
        """从缓存获取（更新 LRU 顺序）"""
        with self._memories_lock:
    ...
    def _cleanup_cache_if_needed(self) -> None:
        """清理缓存到上限以内（温和淘汰：每次最多清理 10%）"""
        with self._memories_lock:
            if len(self._memories) <= self._cache_max_size:
                return

            num_to_remove = max(1, int(self._cache_max_size * 0.1))
            for _ in range(num_to_remove):
                if not self._memories:
                    break
                self._memories.popitem(last=False)

            logger.debug(f"[MemoryManager] 清理缓存到 {len(self._memories)} 条")
...
""".format(timestamp=datetime.now().strftime("%Y-%m-%d %H:%M"))
    ...
    async def on_context_compressing(self, messages: list[dict]) -> None:
        ...
        for fact in quick_facts:
            ...
            with self._memories_lock:
                self._add_to_cache(fact.id, fact)
        if quick_facts:
    ...
    def add_memory(self, memory: Memory, scope: str = "global", scope_owner: str = "") -> str:
        ...
        with self._memories_lock:
            ...
            if vs is not None and vs.enabled:
                similar = vs.search(core_content, limit=3)
            ...
            vs = self._ensure_vector_store()
            if vs is not None:
                vs.add_memory(memory_id, core_content, metadata)

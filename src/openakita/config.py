...
class Settings(BaseSettings):
    ...
    # 桌面应用模式：优化内存占用，懒加载重型模块
    desktop_mode: bool = Field(
        default=False,
        description="桌面应用模式（优化内存占用，懒加载重型模块）",
    )

    # === 搜索后端配置 (v2) ===
    search_backend: str = Field(
        default="api_embedding",
        description="记忆搜索后端：fts5(零依赖) | chromadb(本地向量) | api_embedding(在线 API，推荐，节省内存)",
    )
    ...
    memory_cache_max_size: int = Field(
        default=10000,
        description="记忆缓存最大条目数（LRU 淘汰，0=不限制）",
    )
    memory_cache_ttl_seconds: int | None = Field(
        default=None,  # None = 永不过期
        description="记忆缓存过期时间（秒，None=永不过期）",
    )

    # === Embedding API 缓存配置 ===
    embedding_cache_max_size: int = Field(
        default=5000,
        description="Embedding 内存缓存最大条目数（LRU 淘汰，0=不限制）",
    )
    embedding_cache_ttl_seconds: int | None = Field(
        default=None,  # 永不过期
        description="Embedding 缓存过期时间（秒，None=永不过期）",
    )

    # === Agent 实例池配置 ===
    agent_instance_idle_timeout: int = Field(
        default=21600,  # 6 小时
        description="Agent 实例空闲超时时间（秒，超时后回收）",
    )

    # === 技能系统配置 ===
    skill_lazy_load: bool = Field(
        default=True,
        description="是否启用技能懒加载（节省启动内存）",
    )
    skill_max_loaded: int = Field(
        default=50,
        description="同时加载的最大技能数（LRU 淘汰）",
    )
